...
date: '2024-11-05'
tags: ['python', 'fastapi', 'graphql', 'strawberry graphql', 'relay', 'react', 'javascript']
draft: false
...
- The script takes in `port` and `file` arguments, allowing customization of the server’s listening port and the filename for storing the query data.

2. **Query Handling and Storage:**
...
```bash
python run_persist_server.py --file query_map.json --port 2999
```
