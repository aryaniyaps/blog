...
layout: PostSimple
images: []
summary: This blog post explores how to integrate Relay with Next.js for server-side rendering (SSR), addressing a gap in existing resources. While Relay excels as a GraphQL client for Client-Side Rendering (CSR), this guide demonstrates its potential for SSR use cases, which is vital for applications prioritizing SEO and faster initial load times.
---

GraphQL is an excellent tool for effortless and efficient data fetching in client-side applications, which are mostly Single Page Applications rendered client-side. With the advent of frameworks like Next.js and Remix, the data-fetching paradigm has shifted to support server-side rendering (SSR) where SEO is a top priority.

While Relay is an excellent GraphQL client, it has primarily been employed in Client-Side Rendered (CSR) applications. While Relay works well out of the box with Next.js for CSR, you might wonder if it can also be used for SSR use cases.
...
However, there are no comprehensive guides for integrating Relay with SSR in real-world scenarios, making it a daunting task.
...
This example only covers preloading queries on the server. It does not address other SSR aspects, such as metadata generation, raising `notFound` exceptions before rendering, and more.
...
Before diving into the implementation, it's essential to understand how SSR works.

Server-Side Rendering (SSR) is a technique where a web application's HTML is rendered on the server rather than the client. This allows the server to deliver a fully populated HTML page to the browser, enhancing user experience, especially for users with slower devices or internet connections. SSR is particularly valuable for applications where SEO and faster initial load times are critical.

### Steps of SSR:

1. **Initial Request**  
   The browser sends an HTTP request to the server. The server processes this request, determines the required data, and fetches it.

2. **Data Fetching on the Server**  
   The server retrieves all necessary data (e.g., from a database or an API) before generating the HTML.
...
4. **Sending the Response**  
   The server sends the fully rendered HTML to the browser, which includes the application's initial state.

5. **Hydration**  
   The browser loads JavaScript to "hydrate" the page, linking the static HTML to the client-side React components for interactivity.

## When to Use Server-Side Rendering

While SSR offers significant advantages, you must evaluate whether it is necessary for your use case. This is particularly important when using GraphQL due to its trade-offs:
...
Here is a SSR Page (that displays a job), that preloads queries on the server.
